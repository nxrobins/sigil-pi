...
pub mod policy;
pub mod transaction;
use action::{EntryContract, Step, perform_with_contract};
...
impl Engine {
    pub fn open(config: Config, mode: OpenMode) -> Result<Self> {
        if !matches!(config.version, 3..=6)
            || matches!(config.version, 4 | 6) != config.automatic.is_some()
        ...
        let entry_contract = if matches!(config.version, 5 | 6) {
