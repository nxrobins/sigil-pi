...
#[test]
fn inspection_reports_actual_admitted_owner_without_creating_a_process() {
    let (directory, bridge) = fixture(&script(""));
    let mut pure = PureBridge::new(bridge.config).unwrap();
    assert_eq!(pure.inspect_owner(), Ok(()));
    assert!(pure.worker.is_none());
    assert!(!directory.path().join("seen").exists());
    let owner = pure.admitted.owner;
    pure.admitted.owner = owner.wrapping_add(1);
    assert_eq!(pure.inspect_owner(), Err(Fault::WrongProcess));
    pure.admitted.owner = owner;
    pure.admitted.poisoned = true;
    assert_eq!(pure.inspect_owner(), Err(Fault::CleanupUnconfirmed));
    assert!(pure.worker.is_none());
}

#[test]
fn owner_inspection_does_not_consume_a_call_or_retire_a_usable_worker() {
    let (directory, bridge) = fixture(&script(""));
    let mut pure = PureBridge::new(bridge.config).unwrap();
    ...
    assert_eq!(
        std::fs::read_to_string(directory.path().join("seen"))
            .unwrap()
            .lines()
            .count(),
        1
