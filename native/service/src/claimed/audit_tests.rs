...
fn actual_hard_cancellation_is_projected_without_inventing_a_runtime_result() {
    ...
    f.worker = Bridge::new(Config {
        ...
        max_fuel: 1000,
        max_timeout_ms: HELD_TIMEOUT_MS,
        net: vec![],
        fs: vec![],
        secret_env: BTreeMap::new(),
    })
    ...
    // This fixture stays live until the test's own cancellation kills it, so it
    // asks for the held budget rather than the shared five-second one.
