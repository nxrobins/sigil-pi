...
/// Budgets for fixtures that block until a test releases or cancels them, and for
/// the waits that observe them.
///
/// Such a fixture's own deadline must never be the fault a test observes, and a
/// wait watching it must not expire while a real subprocess is still starting.
/// `Bridge` checks cancellation before the deadline, but that ordering only helps
/// while the lane thread is still running: a request deadline that elapses during
/// fixture setup finishes the thread with `Deadline` before a later `drop` can set
/// the cancel flag. The shared five-second budget was close enough to real setup
/// time that a loaded host lost that race, and the four- and five-second waits
/// expired mid-startup for the same reason. The rest of this suite already allows
/// 15-30s for subprocess work; these bring the lane fixtures to that norm.
/// 30_000ms is the largest value every admission path accepts (`pure::invoke`
/// clamps to 30_000, `Bridge::prepare` to `config.max_timeout_ms`).
pub(super) const HELD_TIMEOUT_MS: u64 = 30_000;
pub(super) const HELD_WAIT: Duration = Duration::from_secs(30);
