...
/// Where an actual native operation failed. This is not application permission,
/// a fallback verdict, retry guidance, or an external-effect delivery status.
...
impl ObservedError {
    fn precheck(error: Error) -> Self {
        Self {
            origin: FailureOrigin::Precheck,
            error,
        }
    }
...
pub type ObservedResult<T> = std::result::Result<T, ObservedError>;
...
impl Store {
    ...
    fn verify_storage_files(&self) -> Result<()> {
        if self.poisoned {
    ...
    pub fn get(&self, scope: &Scope, namespace: &str, key: &str) -> Result<Record> {
        self.get_observed(scope, namespace, key)
            .map_err(|failure| failure.error)
    }

    /// Same operation/check order as get(), with bounded native failure origin.
    /// Precheck failures do not probe storage. A storage-origin failure reports
    /// that this operation passed its scope/process checks, not that a future
    /// action is authorized or any request was successfully accounted.
    pub fn get_observed(
        &self,
        scope: &Scope,
        namespace: &str,
        ...
        self.authorize(scope, namespace, key, false, 0, false)
            .map_err(ObservedError::precheck)?;
        self.verify_process().map_err(ObservedError::precheck)?;
        self.verify_storage_files()
            .map_err(ObservedError::storage)?;
        Self::read_record(&self.connection, namespace, key, self.limits.value_bytes)
            .map_err(ObservedError::storage)
    }
    ...
    /// Check the original caller's batch shape, process/scope authority and
    /// per-batch ceilings without I/O or mutation. This does not check retained
    /// revisions/capacity, reserve a write, or acknowledge a commit. Native
    /// publication couplings use it before adding their separate private scope.
    pub fn check_batch(&self, scope: &Scope, batch: &Batch) -> Result<()> {
        self.validate_batch(scope, batch)?;
        self.verify_process()
    }
    ...
    fn commit_inner(
        ...
        self.commit_observed_inner(scope, batch, before_commit)
            .map_err(|failure| failure.error)
    }

    /// The same transaction used by commit(), with origin attached to failure.
    /// No extra transaction, duplicate validation, retry or weaker acknowledgement.
    pub fn commit_observed(&mut self, scope: &Scope, batch: &Batch) -> ObservedResult<Receipt> {
        self.commit_observed_inner(scope, batch, || {})
    }

    fn commit_observed_inner(
        &mut self,
        scope: &Scope,
        batch: &Batch,
        before_commit: impl FnOnce(),
    ) -> ObservedResult<Receipt> {
        self.validate_batch(scope, batch)
            .map_err(ObservedError::precheck)?;
        self.verify_process().map_err(ObservedError::precheck)?;
        self.commit_storage(batch, before_commit)
            .map_err(ObservedError::storage)
    }

    fn validate_batch(&self, scope: &Scope, batch: &Batch) -> Result<()> {
    ...
    fn commit_storage(&mut self, batch: &Batch, before_commit: impl FnOnce()) -> Result<Receipt> {
        self.verify_storage_files()?;
        let tx = self
            .connection
            .transaction_with_behavior(TransactionBehavior::Immediate)
