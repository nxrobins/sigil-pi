...
use super::*;
use crate::claimed::tests::{
    Fixture, HELD_TIMEOUT_MS, HELD_WAIT, binding, execution, fixture_recorder, recorded_scope,
};
...
fn held_worker(directory: &Path) -> Bridge {
    ...
    let mut config = component(directory, "held", &body);
    config.max_timeout_ms = HELD_TIMEOUT_MS;
...
impl Running {
    fn new(phase: &str, expire: bool) -> Self {
        ...
        if !expire {
            ...
            // The deadline is `Instant::now() + timeout_ms` from the request; the
            // config maximum above is only its ceiling.
            execution.timeout_ms = HELD_TIMEOUT_MS;
        }
    ...
    fn finish(&mut self) -> OwnedCompletion {
        let deadline = Instant::now() + HELD_WAIT;
        loop {
