...
| SIGIL | Commit `8277a1d92d599df89e6b4391fc70fd0fa534d696`, with crates/stdlib tree hashes in `SIGIL_REV`, built with the `solver` feature | Any other or dirty SIGIL source/binary; solver-off builds (they fail closed at the first forge) |
...
| Rust build | rustc 1.98.0 with the pinned SIGIL `Cargo.lock`, plus the Lean toolchain SIGIL pins in `proofs/lean/lean-toolchain` (build-time only; the kernel is statically linked) | Floating stable/nightly toolchains; building without the pinned Lean toolchain |
