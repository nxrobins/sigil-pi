...
def test_blocking_lane_fixtures_cannot_race_their_own_deadline():
    ...
    for name in ("owned/tests.rs", "audit_tests.rs"):
    ...
    owned = (claimed / "owned" / "tests.rs").read_text()
    assert "assert_eq!(observation.fault, Some(Fault::Cancelled));" in owned, (
        "dropping a lane must still be observed as Cancelled; accepting Deadline "
        "here would make a real cancellation regression invisible")
    audit = (claimed / "audit_tests.rs").read_text()
    assert "assert_eq!(observed.fault, Some(Fault::Cancelled));" in audit, (
        "hard cancellation must still be projected as Cancelled, not whatever "
        "fault a slow host produced first")
