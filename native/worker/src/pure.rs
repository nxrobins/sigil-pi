...
impl PureBridge {
    /// Current admitted-owner precheck only. Does not spawn, retry, retire,
    /// inspect application data, or claim a cached child is executing now.
    pub fn inspect_owner(&self) -> Result<(), Fault> {
        self.admitted.live()
    }
