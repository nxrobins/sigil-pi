...
/// Keep the actual admitted pure worker and bound templates across bootstrap;
/// do not re-read a different program when application storage is opened later.
...
impl Admitted {
    pub(crate) fn open(self, store: &Store) -> Result<Transaction> {
        Ok(Transaction {
            ...
            count: self.count,
            inputs: self.inputs,
            scope: store.scope(self.grants).map_err(|_| "config")?,
        })
    }
}
/// Host-created observations of a completed pure producer. Not deserializable
/// and never accepted as an invocation argument or an external-effect result.
...
impl Transaction {
    ...
    pub fn new(config: Config, store: &Store) -> Result<Self> {
        Self::admit(config)?.open(store)
    }

    pub(crate) fn admit(config: Config) -> Result<Admitted> {
        Self::validate_config(&config)?;
        let subject = Subject::new(&config, &config.worker, &config.grants)?;
        Ok(Admitted {
            subject,
    ...
    pub fn apply(
        ...
        let plan = self.prepare(store, last_clock, values)?;
        let receipt = match plan.batch {
            Some(batch) => {
                admit_time(None, last_clock, plan.deadline, now)?;
    ...
    pub(crate) fn prepare(
        ...
        let deadline = started + Duration::from_millis(self.function.timeout_ms);
        ...
        let evaluated = self.function.observe(input, deadline);
        self.pending_evaluation = Some(Pending::new(evaluated.facts, lookup, deadline));
        let (output, selected_timeout) = evaluated.result?;
        ...
        let facts = vec![
            ...
            batch.as_ref().map_or(0, |b| b.checks.len()).to_string(),
            batch.as_ref().map_or(0, |b| b.writes.len()).to_string(),
        ];
        self.pending_evaluation = None;
        Ok(Prepared {
            context,
