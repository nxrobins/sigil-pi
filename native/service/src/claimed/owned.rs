...
impl Attempt<'_> {
    fn handoff(mut self, recorder: &mut Recorder) -> Result<Handoff> {
        ...
        let bound = Handoff {
            ...
            refused,
            prepared_audit: self.prepared_audit.clone(),
        };
...
impl OwnedWorker {
    pub(crate) fn inspect_owners(&self) -> Result<usize> {
        self.recorder.inspect_owner()?;
        if let Some(worker) = &self.worker {
            worker
                .facts()
                .map_err(crate::runtime_inspection::owner_fault)?;
        ...
        if self.flight.is_some() {
            // The actual owner is in the effect thread or its uncollected
            // report. Never join, retry, cancel, or invent an idle-owner check.
            return Ok(1);
        }
    ...
    pub fn start(
        ...
        // Retire previous observations before even a busy/owner gate. The inner
        // Result owns no Attempt/Store borrow when the failure is published.
        policy.discard_evaluation();
        let result = self.start_inner(policy, store, last_clock, values);
        if let Err(code) = result
            && let Some(failure) = policy.take_failure(code)
        {
            self.recorder.record_evaluation_failure(store, failure)?;
        }
        result
    }
    ...
    fn record(
        ...
        let proposal = self.recorder.propose_bound(
            store,
            BoundRecords {
                intent: &bound.intent,
                ...
                prepared_audit: Some(&bound.prepared_audit),
            },
            &facts,
            deadline,
        )?;
        ...
        let receipt =
            self.recorder
                .commit(store, &self.scope, &proposal, &batch, deadline, None)?;
