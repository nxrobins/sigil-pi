"""Build-only composition of the fixed SIGIL worker-lifecycle audit policy."""
import hashlib
...
def compose_effect_audit(repo, stdlib):
    base = compose_transaction_audit(repo, stdlib)
    # Reuse its exact pure record/JSON/audit helpers, never its TA1 entry point.
    ...
    prefix = prefix.replace("module pi_transaction_audit;", "module pi_effect_audit;")
    path = ROOT / "app/pi/effect_audit.sigil"
    raw = path.read_bytes()
    ...
    inputs = {**base.input_hashes,
        "app/pi/effect_audit.sigil": hashlib.sha256(raw).hexdigest(),
        "scripts/compose_effect_audit.py": hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
